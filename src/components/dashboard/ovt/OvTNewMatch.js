...
import { addOvTMatch } from '../../../store/actions/addMatch';
import { scoreOvTUpdate } from '../../../store/actions/scoreUpdate';
import { opponentOvT } from '../../../store/actions/opponent';
import { addPairOvT } from '../../../store/actions/addPlayer';
...
class TvTNewMatch extends Component {
	...
	setDisabled = () => {
		let winDisabled = false;
		let loseDisabled = false;
		if(this.state.loser2 !== ''){
			winDisabled = true
		}
	...
	handleSubmit = (e) => {
		...
		if (
			...
			this.props.addOvTMatch(this.formatState(this.state));
			this.props.scoreOvTUpdate(this.setUpdate());
			this.props.opponentOvT(
			...
			for (let i = 0; i < this.props.tvTplayers.length; i++) {
				if (
					...
					this.props.addPairOvT(this.newPair());
					break;
				}
			}
		}
			...
			<div className="newMatchCard center-align">
				...
				<div className="row">
					...
					<Select
						...
						isDisabled={this.setDisabled().winDisabled}
					/>
					<Select
						className="col s6"
						options={playrNameList}
						...
						isDisabled={this.setDisabled().loseDisabled}
					/>
				</div>
...
const mapDispatchToProps = (dispatch) => {
	return {
		addOvTMatch: (match) => dispatch(addOvTMatch(match)),
		scoreOvTUpdate: (updatedPlayers) => dispatch(scoreOvTUpdate(updatedPlayers)),
		opponentOvT: (opponents) => dispatch(opponentOvT(opponents)),
		addPairOvT: (pair) => dispatch(addPairOvT(pair))
	};
};
